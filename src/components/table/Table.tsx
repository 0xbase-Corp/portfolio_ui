     ...
     <Table aria-label="portfolio table" sx={{ backgroundColor: 'background.default' }}>
        <TableHead>
          <TableRow>
            ...
            <TableCell align="left">Asset Price (USD)</TableCell>
            <TableCell /> 
          </TableRow>
        </TableHead>
        <TableBody>
          {dummyData.map((row) => (
           ...
           <StyledTableRow level={0}>
                <TableCell>
                  <IconButton size="small" onClick={() => handleToggle(row.asset_name)}>
                  {open[row.asset_name] ? <ArrowDropUpIcon /> : <ArrowDropDownIcon />}
                  </IconButton>
                </TableCell>
                <TableCell component="th" scope="row">{row.asset_name}</TableCell>
    ...
    <Box display="flex" alignItems="center" justifyContent="flex-start">
      ...
      <Box marginLeft={0.5}>{`${row.profit_loss}%`}</Box>
              ...
              {row.subRows && row.subRows.map((subRow, subIndex) => (
                <StyledTableRow key={`${row.asset_name}-${subIndex}`} style={{ display: open[row.asset_name] ? 'table-row' : 'none' }} level={1}>
                  <TableCell />
                  <TableCell component="th" scope="row" style={{ paddingLeft: '50px' }}>
                    <Typography variant="body2" color="textSecondary">{subRow.asset_name}</Typography>
                  </TableCell>
                  ...
                  <TableCell align="left">{`${subRow.percentage}%`}</TableCell>
